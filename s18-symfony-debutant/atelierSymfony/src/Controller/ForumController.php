...
class ForumController extends AbstractController
{
    ...
    public function subject(int $id,Request $request, ValidatorInterface $validator): Response
    {
        $errors = [];
        $subjectRepository = $this->getDoctrine()->getRepository(Subject::class);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
          ...
          if(count($errors) === 0) {
            ...
            $entityManager->flush();
            $this->addFlash('success','Votre Réonse a bien été enregistrée');
          }
        }
    ...
    public function newSubject(Request $request, ValidatorInterface $validator): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
          //On associe le sujet à l'utlisateur connecté
